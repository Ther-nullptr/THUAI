...
#define MOVETIME_WALLS 50 //躲墙的速度
#define MOVETIME_ENEMY 300 //躲敌人的速度
#define MOVETIME_ESCAPE 1000 //躲敌方子弹的速度
#define MOVETIME_RANDOM 50 //随机游走的速度
#define MOVETIME_DIRECTION 50 //定向移动速度
#define MOVETIME_BIRTHPOINTS 70 //躲出生点的速度
#define BULLET_ATTACK 700 //攻击时子弹的飞行速度
#define BULLET_COLOR 700 //染色时子弹的飞行速度

#define BIRTHPOINT1 2 // 改变方向所用的参考值
#define BIRTHPOINT2 47 // 
#define MIN 1 // 
#define MAX 48 // 
#define MIDDLE 25*CELL // 地图中心的位置
...
uint32_t shootPositionX = 0, shootPositionY = 0;
...
#ifdef FILEMODE
std::ofstream fout("C:\\Users\\86181\\Desktop\\game2.txt");
#endif // FILEMODE
...
void AI::play(GameApi& g)
{
	...
	std::cout << shootPositionX << ',' << shootPositionY << std::endl;
	std::cout << getDistance(shootPositionX, shootPositionY, selfPositionX, selfPositionY) << std::endl;
	// 此处小鱼人的攻击难以被优化,所以我们约定:小鱼人的染色子弹飞行方向永远指向中心,同时注意要换位置进行发射
	if (getDistance(shootPositionX, shootPositionY, selfPositionX, selfPositionY) > 3000)
	{
	...
	{
		fout << "PurpleFish has been slained at " << time << " s\n";
	}
#endif // FILEMODE
